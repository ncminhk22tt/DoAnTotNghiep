...
function resolveAvatarSrc(value: string): string {
...
export default function PatientProfilePage() {
  ...
  async function handleAvatarFileChange(file: File | null) {
    ...
    if (!allowed.includes(file.type)) {
      showToast("Ảnh đại diện chỉ hỗ trợ png/jpg/webp", "error");
      return;
    }
    ...
    try {
      ...
      showToast(err instanceof Error ? err.message : "Không thể tải ảnh đại diện", "error");
    }
  }
      ...
      {activeForm === "profile" ? (
        <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 12, padding: 16 }}>
          <form onSubmit={onSubmit} style={{ display: "grid", gap: 10 }}>
            ...
            {form.avatar ? (
              <img
                src={resolveAvatarSrc(form.avatar)}
                alt="Ảnh đại diện"
                style={{
                  width: 120,
                  height: 120,
